...
from clean_jax_exp.posthoc_analysis import ensure_sample_quality_metrics, ensure_sample_subspace_metrics
...
def config_from_checkpoint(checkpoint: dict[str, Any]) -> TorchMixerConfig:
    cfg = dict(checkpoint["model_config"])
    allowed = TorchMixerConfig.__dataclass_fields__.keys()
    return TorchMixerConfig(**{k: cfg[k] for k in allowed if k in cfg})
...
def run_experiment(args: argparse.Namespace):
    ...
    if args.run_name:
        ...
        run_name = (
            f"torch_mixer1d_resume_D{model_cfg.ambient_dim}_adamw_p{model_cfg.patch_size}_d{model_cfg.dim}"
            f"_L{model_cfg.depth}_tm{model_cfg.token_mlp_width}_cm{model_cfg.channel_mlp_width}"
    ...
    metadata = {
        ...
        "patch_count": model_cfg.ambient_dim // model_cfg.patch_size,
