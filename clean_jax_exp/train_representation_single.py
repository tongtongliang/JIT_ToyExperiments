...
from .models_single import SingleFCNModelConfig, count_params, forward, init_params, loss_fn
from .posthoc_analysis import ensure_sample_quality_metrics, ensure_sample_subspace_metrics
from .train_gradient import AnalysisConfig, checkpoint_dump
...
def make_training_functions(model_cfg: SingleFCNModelConfig, train_cfg: RepresentationTrainConfig, mode: str):
    def body(carry, _):
...
def train_mode(mode: str, init_params_tree, x0_all, model_cfg: SingleFCNModelConfig, train_cfg: RepresentationTrainConfig, seed: int):
...
def collect_repr_for_batch(params, z_t_np: np.ndarray, t_np: np.ndarray, model_cfg: SingleFCNModelConfig, batch_size: int):
...
def run_representation_analysis(run_dir: Path, params_by_mode: dict[str, Any], x0_np: np.ndarray, model_cfg: SingleFCNModelConfig, cfg: AnalysisConfig):
    rows = []
...
def run_stability_analysis(run_dir: Path, params_by_mode: dict[str, Any], x0_np: np.ndarray, model_cfg: SingleFCNModelConfig, cfg: AnalysisConfig):
    rows = []
...
def make_sampler(model_cfg: SingleFCNModelConfig, mode: str, t_min: float, n_steps: int):
...
def run_sampling_analysis(
    ...
    model_cfg: SingleFCNModelConfig,
...
def run_experiment(args: argparse.Namespace):
    ...
    run_name = args.run_name or f"repr_single_D{args.ambient_dim}_adamw_w{args.width}_d{args.depth}_s{args.steps}_seed{args.seed}_{ts}"
    ...
    metadata = {
        ...
        "modes": list(modes),
        "experiment_type": "representation_training_single_linear_fcn",
        "optimizer": "adamw_manual",
        "model_family": "single_linear_residual_fcn_adaln_zero",
        "model_config": asdict(model_cfg),
        "parameter_count": count_params(init),
        "block": "h <- h + alpha(t) * relu(W * AdaLN(h,t))",
        "train_config": asdict(train_cfg),
        "analysis_config": asdict(analysis_cfg),
        "notes": "Flow-matching representation/sampling run for the one-hidden-linear residual AdaLN-zero FCN. Default depth=10 matches the old 5-block FCN's hidden linear count.",
    }
    (run_dir / "metadata.json").write_text(json.dumps(metadata, indent=2), encoding="utf-8")
    np.savez_compressed(run_dir / "training_data_snapshot.npz", **data)

    print(f"Single-linear FCN parameter count: {metadata['parameter_count']:,}", flush=True)
    x0_all = jnp.asarray(data["x0"], dtype=jnp.float32)
    params_by_mode: dict[str, Any] = {}
    all_loss_rows: list[dict[str, Any]] = []

    for mode in modes:
        print(f"\n{'=' * 90}\nSingle-linear FCN flow-matching training mode={mode}\n{'=' * 90}", flush=True)
    ...
    print(f"\nSingle-linear FCN representation run complete: {run_dir}", flush=True)
    return run_dir


def build_argparser():
    p = argparse.ArgumentParser(description="Long-training single-linear residual FCN flow-matching experiment")
    p.add_argument("--output-root", default="results/clean_jax_representation_single")
